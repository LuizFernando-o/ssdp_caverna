...
public class SELECAO {
    ...
    /**
     * Retorna índices vencedores em torneios binários (entre dois OU MAIS indivíduos)
     * 
     * @author Time Caverna do Dragão
     * @param tamanho
     * @param P                população
     * @return int[] - índices vencedores de P
     * @since 27/01/2016
     * @version 1.0
     */
    public static int[] torneioBinarioComTamanhoTorneio(int tamanho, Pattern[] P, int tamanhoTorneio) {
        ...
        for (int i = 0; i < tamanho; i++) {
            // Seleciona tamanhoTorneio indivíduos aleatórios
            int[] concorrentes = new int[tamanhoTorneio];
            for (int j = 0; j < tamanhoTorneio; j++) {
                concorrentes[j] = indices[Const.random.nextInt(tamanho)];
            }
            // Encontra o índice do indivíduo com maior aptidão
            int idxMelhor = concorrentes[0];
            for (int j = 1; j < tamanhoTorneio; j++) {
                if (P[concorrentes[j]].compareTo(P[idxMelhor]) > 0) {
                    idxMelhor = concorrentes[j];
                }
            }
            selecionados[i] = idxMelhor;
        }
        return selecionados;
    }
    ...
    public static int salvandoRelevantesDPmais(Pattern[] Pk, Pattern[] PAsterisco, double similaridadeLimite) {
        ...
        for (int i = 0; i < PAsterisco.length
            ...
            // (3) similar
            // (3.1) similar com p_Pk maior: p_Pk engloba como similar e não muda a ordem
            // das DPs em Pk
            // (3.2) similar com p_PAsterisco maior: p_PAsterisco engloba como similar p_Pk,
            // ocupa a vaga em Pk[i] e reordena-se Pk
            for (int j = 0; j < Pk.length; j++) {
                ...
                } else if (j == Pk.length - 1) {// Se dp.new não for similar a nenhuma DP de Pk, então ele substitui a
                                                // última
